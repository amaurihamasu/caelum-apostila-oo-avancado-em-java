...
public class LeitorOpcoesCLI implements ParametrosCotuba {
	...
	private Option opcaoDeDiretorioDosMD() {
		return new Option("d", "dir", true, "Diretório que contem os arquivos md. Default: diretório atual.");
	}

	private Option opcaoDeFormatoDoEbook() {
		return new Option("f", "format", true, "Formato de saída do ebook. Pode ser: pdf ou epub. Default: pdf");
	}

	private Option opcaoDeArquivoDeSaida() {
		return new Option("o", "output", true, "Arquivo de saída do ebook. Default: book.{formato}.");
	}
